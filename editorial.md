...
* ![2](https://user-images.githubusercontent.com/4417431/159480764-988028d3-5238-42f2-981f-8e25d6a47aa8.png)번 문제와 ![8](https://user-images.githubusercontent.com/4417431/159481340-8269345b-2208-4ab4-9291-8be95fcb5e09.png)번 문제가 스코어보드를 치열하게 만들어줬습니다.

## ![1](https://user-images.githubusercontent.com/4417431/159480297-0b813cb4-d176-47c8-aa5d-50ba1b943c48.png)번. 한별 찍기
_출제자: shiftpsh,_
_최고 득점자: mirusu400 (14초, 222,222점)_

예제에 주어진 한별이를 그대로 출력하면 됩니다.

브라우저에 따라 예제 출력이 그림으로 보일 수 있지만, 실제로는 작은 크기의 폰트로 작성된 아스키 아트이기 때문에 그대로 복사해서 출력하면 됩니다. 가장 쉬운 방법은 언어 설정을 Text로 하고 복사-붙여넣기하여 제출하는 것입니다.

Text 외의 언어로 풀 경우, 공백이 예제 출력과 정확히 같게 출력해야 함에 유의합시다.
...
## ![6](https://user-images.githubusercontent.com/4417431/159481232-3aa3fdf6-af20-47e2-8a8f-6f316b091f06.png)번. 24905번 문제
_출제자: shiftpsh,_
...
풀이는 Pearson, David. "A polynomial-time algorithm for the change-making problem." *Operations Research Letters* 33.3 (2005): 231-234.입니다. 여기에 설명하기는 조금 길기 때문에, 문제를 풀고 싶으신 분들은 직접 찾아 보시기 바랍니다.

데이터를 강하게 만드는 게 매우 힘들었는데, 여섯 개 정도의 논문을 더 읽은 결과 현재 데이터셋에는 다음과 같은 데이터가 모두 들어 있습니다:

- 마지막에서 두 번째 동전까지는 답이 `Yes`이나, 마지막 동전 때문에 `No`를 찍고 반례를 찾아야 하는 케이스
- 답이 비자명하게 `Yes`인 케이스
  - 비자명하다는 것은 약수·배수 관계이거나 등차수열을 이루는 액면가의 동전이 없고 액면이 충분히 커서 [액면에 대한 DP 풀이](https://www.acmicpc.net/problem/13137)를 막을 수 있다는 뜻입니다.
  - 이 종류의 테스트 케이스는 **시간 제한**을 테스트하기 위해 상당히 많이 들어 있습니다.
- 답이 `No`이나 c · a<sub>i</sub>가 절대로 반례가 되지 않는 케이스
- 답이 `No`이나 (a<sub>i</sub> + a<sub>j</sub>)가 절대로 반례가 되지 않는 케이스
- 답이 `No`이고 최소 반례 구성에 *많은* 종류와 *많은* 수의 동전을 반드시 써야 하는 케이스

안타깝게도 이렇게 데이터를 때려넣었음에도 **데이터가 여전히 약합니다.** 따라서 이 문제는 구데기컵의 기본 규칙과 다르게 **데이터 추가를 환영**합니다.
...
_최고 득점자: sait2000 (742분, 142,222점)_
...
온도는 영하 273.15도 밑으로 내려갈 수 없기 때문에, n <= 27에 대해서만 N-퀸 문제를 풀면 된다. 우연히도 딱 여기까지 정확한 답이 알려져 있으므로, 이를 하드코딩하고 116으로 나눈 몫을 출력하면 된다. 10<sup>9</sup>+7이 아니라 109+7이고, 나머지가 아니라 몫이라는 점을 유의하자.

### 문제 4

_출제자: jh05013_

여러 방법으로 풀 수 있다. 출제자의 풀이는 다음과 같다.
- EOF가 들어올 때까지 공백 단위로 문자열을 입력받는다.
- 문자열의 길이가 3이고 두 번째 글자가 쉼표이면, 첫 번째와 세 번째 글자를 각각 수로 변환한다. 아닐 경우, 문자열 전체를 그대로 수로 변환한다.
- 수가 홀수 개 존재하면 첫 번째 수를 지운다.
- 마지막 수가 0이면 마지막 두 수를 지운다.
- 두 수마다 합을 구해 출력한다.

모든 A+B 문제를 사용하지 않은 이유는, A+B 문제를 더 넣으면 한 입력이 여러 문제에 대응되는 경우가 생기기 때문이다. 이는 스페셜 저지를 더 복잡하게 만들기도 하고, 참가자 입장에서는 중복되는 문제를 그냥 무시해도 되기 때문에 난이도를 그다지 증가시키지도 않는다.
...
`그리고`, `을`이나 `공백` 등을 따로 더 곱해 출력해야 하는 함정도 없고, 정말로 문제를 **잘 읽으면** 무난하게 풀 수 있었던 문제였습니다. 다만 이 문제의 입력 란은 약 2만 개의 글자로 이루어져 있고, 웬만한 소설책의 글자 수가 20만 자이니 직접 읽기는 쉽지 않았을 것입니다.

우리 앞에는 컴퓨터가 있으니 이 "1/10 소설"을 조금 더 잘 읽어 봅시다. 일단 입력을 모두 복사해서 배열로 만듭니다.
...
그런 다음 출력에 사용되는 글자들만 쭉 이어서 찾아 봅시다. 출력에 사용되는 글자는 총 182자이고, 이 정도는 충분히 심혈을 기울여 읽을 수 있습니다.

```py
>>> len(S)
182
>>> ''.join(x for i, x in enumerate(input_desc) if x in S)
'궁효각독창태리방화입음구향잠회억최군장시식교초잉동휘압분내도광집개우유합전중득복신단통학설피작국정메한쟁원결서영친파별성경예현항획병지그직해게변번불양약응가력횡부레용활고트율물저법폭포벽협적괄석실괴념표미극금찰생행발자술세배로주어진다투청격연심핵점필공본달사일운수위이책풍소호견상무대조만여담노의반형능편돌순육제재천치출관완체탄감권근악비흡스판마문강충겸인추매기속'
```

뭔가 말이 될 것 같으면서 말이 안 되는 단어들이 이어지는데... 잘 읽어보시면 중간쯤 **세배로주어진다**가 눈에 띕니다. 이것이 의도되지 않았다기엔 일곱 글자인 "세 배로 주어진다"가 너무 명확하게 보이므로, 이 글자들의 위치를 검색해 봅시다.

```py
>>> [(i, x) for i, x in enumerate(input_desc) if x in set('세배로주어진다')]
[(7665, '세'), (7666, '배'), (7667, '로'), (7668, '주'), (7669, '어'), (7670, '진'), (7671, '다')]
```

놀랍게도 이 글자들의 위치가 7 665부터 7 671까지 연속해 있습니다! 무슨 문장을 전달하려고 하는 게 틀림없습니다. 이 문장을 실제 디스크립션에서 Ctrl+F로 검색해 봅시다. 각 글자가 `, `로 구분되어 있으므로 `세, 배, 로, 주, 어, 진, 다`를 검색하면 되겠죠.

![술은 세 배로 주어진다](sul-thrice.png)

숨겨진 메세지를 찾았습니다! `술`(0번째부터 시작했을 때 7 663번째 글자)이 세 배로 주어지기 때문에, 입력을 받은 다음에 **7 663번째 인덱스를 3으로 나누고** 요구한 단어에 속한 글자의 값을 곱해서 출력하면 정답을 받을 수 있습니다.

참고로, 출제자는 11 172자를 모두 읽으면서 혹여나 다른 부분에서 의심스러운 문장이 만들어지지는 않는지 검사했습니다. 여러분도 문제가 틀렸다고 하기 전에 디스크립션부터 다시 **꼼꼼히** 읽어보는 습관을 들입시다.
...
구데기컵은 매년 많은 분들의 도움으로 이루어지고 계시지만, 특히 백준님이 일반적인 대회보다 훨씬 특이한 문제를 위해 고생하시는 것에 비해 그 수고가 충분히 알려지지 않는 것 같아 이 문제를 만들었습니다.

여러분들의 구데기컵에 대한 관심사도 알아볼 수 있었는데, 정답이 [구데기컵 트위터](https://twitter.com/ghudegy_cup/status/1321904526174035973)에 있기 때문입니다. 이 트윗은 에디토리얼 작성 시점 기준 리트윗 0, 마음 0이었기 때문에 가장 잘 알려져 있지 않았을 거라고 추정했습니다. 트윗이 있다는 것을 알지 않고서는 `출제할 마음은 딱히 없지만 출제에 도움을 주거나 문제의 내막을 모르는 검수진들이 검수할 수 없는 내부 코드를 검수하기 위해서 출제와 관련된 채널에 들어가 있어야 하는 사람`을 맞힐 수는 없었을 것입니다.
...
_최고 득점자: sait2000 (830분, 142,222점)_
...
- (4) [Shibuya emoji](https://emojipedia.org/shibuya/)로, 대부분의 브라우저에서 제대로 표시되지 않을 것이다. 초기에 만들어졌으나 유니코드 표준으로 채택되지 않아 자취를 감춘 이모지이지만, 트위터는 이 이모지를 꾸준히 지원하고 있다.
